...
日本語で出力する成果物（`/guide` の HTML 本文、`/research` の Markdown 本文）に適用する文章規範。

出力言語が日本語のときだけ適用する。英語で出力するときは [document-quality.md](document-quality.md) のみに従う。日本語出力時は document-quality.md とこのファイルの両方に従い、重複する観点（簡潔さ、平易な言葉、強調の節度）は document-quality.md を土台に、ここで日本語固有の規範を上乗せする。

## 適用範囲メモ

- このファイルの規範の大半（段落と論証の構成、論証の厳密さ、読み手の負荷、視点と語り、演出の抑制、LLM っぽい表現の禁止、冗長の排除、見出しの付け方、読者への誠実さ）は、HTML と Markdown の両方の日本語本文に効く。
- 「整形」節は元々ソース原稿向けの規範である。`/research` の Markdown 本文にはそのまま適用する。`/guide` の HTML 出力では、レンダリング後の見た目に効くもの（太字での用語定義、ダッシュ・中黒の扱い、見出しの付け方）だけを適用し、「一文ごとに改行」「脚注 `[^ラベル]`」のようなソース整形は HTML には適用しない。
