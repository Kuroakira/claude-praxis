...
Dispatch a `general-purpose` Task subagent with a self-contained task prompt. The prompt follows the same G1 planning pipeline template as `commands/implement.md` (see "G1: Full Planning Pipeline" section), with these modifications:
