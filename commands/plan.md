...
>
> **Skip criteria for Scout**: Scout may be skipped ONLY when: (a) the change targets a single file explicitly specified with no cross-module integration points, or (b) a Scout was dispatched in the immediately preceding task covering the same codebase area. When skipping, state the specific reason in the plan header. Generic reasons ("scope is clear", "straightforward change") are not sufficient — name the file and explain why no unknown patterns exist.
>
> **Skip criteria for best-practices**: Best-practices research may be skipped ONLY when the implementation uses no external frameworks or libraries (pure language-only logic). When skipping, state the reason. If ANY framework or library is involved (even well-known ones like React, Express, Vitest), best-practices research is mandatory — internal knowledge may not reflect recent API changes or deprecations.
>
...
> - Common axes: test strategy, implementation ordering, pre-implementation refactoring scope (consolidation vs. add-then-refactor), dependency management, error handling approach
>
...
> 4. **Refactoring-first tasks** (MANDATORY evaluation): Before creating feature tasks, evaluate the system holistically for refactoring opportunities — not just within the target scope, but across the entire system where the new feature's concerns overlap. Two sources of refactoring tasks:
>>
>>    **a) Functional overlap analysis** (always): Identify existing code that overlaps with, duplicates, or is similar to the planned feature. If patterns exist that should be consolidated, generalized, or restructured to naturally accommodate the new feature, create refactoring tasks BEFORE feature tasks. The goal: make the codebase ready to receive the feature cleanly, rather than forcing the feature into a structure that wasn't designed for it. Look for: duplicate logic that the new feature would add a third instance of, similar patterns that should be unified, existing structures that fight the new feature's shape. If no functional overlap is found, state "functional overlap reviewed — no consolidation needed" in the plan header.
>>
>>    **b) Health baseline** (when D/F dimensions exist): If D/F dimensions from `health_baseline` overlap with the implementation scope, create refactoring tasks targeting those areas. Each health-based refactoring task should: (a) target a specific D/F dimension and its affected files, (b) include verification that `mcp__plugin_sekko-arch_sekko-arch__health` shows improvement after the refactoring, (c) be self-contained. If D/F dimensions do not overlap with the implementation scope, note "health baseline reviewed — no pre-implementation refactoring needed" in the plan header.
>>
>>    **c) Pattern consolidation** (always): Read the architecture-analysis report from Step 2 (specifically its Structural Observations and, if present, Pattern Opportunities section). If the report includes a Pattern Opportunities section, use those findings directly. If it does not, read `catalog/structural-pattern-review-points.md` and evaluate the report's friction areas and structural observations for design pattern applicability — matching analysis findings (not raw code) against catalog recognition signals. Look for: scattered type-dispatching that the new feature would add branches to, state-dependent behavior that the feature would complicate, responsibility mixing that the feature would worsen. If pattern consolidation is warranted, create refactoring tasks BEFORE feature tasks (e.g., "Extract Strategy for payment types before adding new payment method"). If no pattern opportunities are found, state "pattern consolidation reviewed — no restructuring needed" in the plan header.
>>
