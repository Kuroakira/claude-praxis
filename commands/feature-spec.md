...
Invoke `check-past-learnings` (role: requirements). Carry relevant learnings forward as context for the interview.
...
**Record to `.claude/context/progress.md`**: Append an entry with the key decisions captured in the spec. Include scope decisions from Phase 2 (In/Out Scope choices and their reasoning) — these entries serve as comparison material for `/understanding-check`.
