...
|------|--------------|-----------------|
| Record | Only `/praxis:compound` writes to learnings | Compound rarely runs → nothing is recorded |
| Recall | Only `/praxis:implement` reads learnings | Design and debug don't check past knowledge |
...
Add to `/praxis:design` and `/praxis:debug`:
...
`/praxis:implement` already does this. The same pattern applies to all three.

### 3. Artifact Persistence (Survive compact)

| Artifact | Save Location | When |
|----------|--------------|------|
| Design Doc | `design-docs/[name].md` | End of `/praxis:design` Phase 4 |
| Investigation Report | `.claude/context/investigation-[name].md` | End of `/praxis:debug` Phase 4 |
| Implementation Plan | `.claude/context/task_plan.md` | End of `/praxis:implement` Phase 1 |
