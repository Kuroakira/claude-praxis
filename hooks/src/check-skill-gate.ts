...
try {
  ...
  exitDeny(
    "check-skill-gate: JSON parse failed. Input may be malformed. Disable the claude-praxis plugin temporarily if this persists.",
  );
}
...
if (!sessionId) {
  exitDeny(
    "check-skill-gate: session_id extraction failed. Cannot verify skill gate. Disable the claude-praxis plugin temporarily if this persists.",
  );
}
...
const requiredSkill =
  fileType === "code" ? "code-quality-rules" : "document-quality-rules";
...
const ruleFile =
  fileType === "code" ? "rules/code-quality.md" : "rules/document-quality.md";
writeJson({
  hookSpecificOutput: {
