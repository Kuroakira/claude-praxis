...
export function trimProgressFile(filePath, maxEntries) {
    if (!fs.existsSync(filePath))
        return;
    const content = fs.readFileSync(filePath, "utf-8");
...
export function updateCompactTimestamp(filePath) {
    if (!fs.existsSync(filePath))
        return;
    const content = fs.readFileSync(filePath, "utf-8");
    const timestamp = new Date().toISOString().replace(/\.\d{3}Z$/, "Z");
    const lines = content.split("\n");
    const lastCompactedIndex = lines.findIndex((line) => line.startsWith("Last compacted:"));
