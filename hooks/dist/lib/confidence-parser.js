const CONFIRMED_RE = /^- \*\*Confirmed\*\*:\s*(\d+)回\s*\|\s*(\d{4}-\d{2}-\d{2})\s*\|\s*(.+)$/;
export function parseConfirmed(line) {
...
export function formatConfirmed(meta) {
    return `- **Confirmed**: ${meta.count}回 | ${meta.lastConfirmed} | ${meta.phases.join(", ")}`;
}
export function createInitial(phase, date) {
    return { count: 1, lastConfirmed: date, phases: [phase] };
}
export function incrementConfirmed(existing, phase, date) {
...
export function mergeConfirmed(entries) {
    if (entries.length === 0) {
...
export function parseFileConfidence(content) {
