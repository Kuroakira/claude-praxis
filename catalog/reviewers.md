...
- **Prompt**: Review for code quality. First, read `catalog/code-quality-review-points.md` — this is your checklist with 29 concrete review points across 10 categories: (1) YAGNI, (2) DRY, (3) KISS, (4) Test quality, (5) Dead code/tech debt, (6) Pattern consistency, (7) Abstraction level discipline, (8) Naming and convention consistency, (9) Responsibility and coupling, (10) Function interface design. Also check compliance with `rules/code-quality.md` (TDD, type safety, no lazy assertions). For each changed file, systematically check every applicable point. When you find a violation, cite the specific point ID (e.g., "4-2: async await leak", "9-1: feature envy") and the committer principle. Prioritize findings by severity: high = correctness or test reliability risk, medium = maintainability concern, low = convention.
