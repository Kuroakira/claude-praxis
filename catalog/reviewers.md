...
- **Verification Source**: `claudedocs/research/ts-review-insights.md` (curated review patterns from TanStack/query, zod, trpc, zustand/jotai, microsoft/TypeScript)
- **Applicable Domains**: implement, debug
- **Prompt**: Review TypeScript code quality using insights from top OSS committers. Read `claudedocs/research/ts-review-insights.md` for the full checklist and pattern reference. Check: (1) Type safety: Is `any` used where `unknown` + type guard would work? Are there `as` assertions that indicate broken type inference? Do generics have the right number of parameters for inference to flow naturally? Are type guard functions named for semantic intent, not structural check? (2) Performance: Are closures allocated in hot paths? Do type-only changes affect bundle size? Are changes minimal in performance-critical code? (3) API design: Are new features opt-in (not breaking existing behavior)? Are internal types marked `@internal`? Is `let` used where `const` + closure would work? (4) Test quality: Do tests verify behavior, not implementation details? Is there a test that fails on main but passes on this PR? Are related syntax patterns also tested? (5) PR discipline: Is the diff minimal? Are logic changes and type changes separated? Is the PR scoped to the original issue? For each finding, cite the specific committer principle that applies.
